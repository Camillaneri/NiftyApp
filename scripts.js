...
function getTask() {
    ...
    document.getElementById("changeMe").classList.add("overflow-scroll");
    ...
    .then((result) =>{  
        ...
        img = document.querySelectorAll('img.img-thumbnail')[i];//get the div where imgs are(should we create them?)
        //console.log(img)
        img.src = img_url;
        img.id = img_id;//put id and url in img
    }
        i++;
        ;}}
    )  
...
//ADD IMAGES TO QUERY //aggiungere condizione che non permetta all'utente di mettere stessa immagine più volte
...
<div class="position-relative col-3 p-0" id="item1">

     <div class="col-2 position-absolute">
         <input class="btn btn-light p-0" style="font-family: bootstrap-icons" type="button" id="imgBtn" onclick="AddLiked(event)" value="&#xF407;"> 
         <input class="btn btn-light p-0" style="font-family: bootstrap-icons" type="button" id="imgBtn" onclick="AddDisliked(event)" value="&#xF405;">
     </div>

      <img id="dragData3" src="https://ipfs.pixura.io/ipfs/QmW3XevHttaTBQ9CwVYtTE8e92kpmCJW4FysGncSfPDBee/screaminSkull-final-opt.gif" class="img-thumbnail" alt="..." draggable = “true” ondragstart="drag(event)">
  </div>
    ...
    div0.className ="col-2 position-absolute"

    const div1 =  document.createElement('div');
    div1.className ="position-relative col-3 p-0";
    div1.id = "item1";

    const input0 = document.createElement('input');
    input0.className ="btn btn-light p-0"
    input0.style = "font-family: bootstrap-icons"
    input0.type = "button"
    input0.id = "imgBtn"
    input0.onclick = "AddLiked(event)"
    input0.value = "&#xF407;"

    //<input class="btn btn-light p-0" style="font-family: bootstrap-icons" type="button" id="imgBtn" onclick="AddLiked(event)" value="&#xF407;">

    const input1 = document.createElement('input');
    input1.className ="btn btn-light p-0"
    input1.style = "font-family: bootstrap-icons"
    input1.type = "button"
    input1.id = "imgBtn"
    input1.onclick = "AddDisliked(event)"
    input1.value = "&#xF405;"
